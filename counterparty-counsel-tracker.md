...
| ID | Question | Why it matters | Working assumption | Gate | Status |
|---|---|---|---|---|---|
| C-PR-1 | **User ownership** — can wallets be configured so the user is the sole owner with exclusive owner permissions? | Self-custody / no-control analysis. | User is sole owner. | Pilot blocker | Open — supportive evidence, config-level confirmation needed: overview deck (9/14) claims "flexible custody — both non-custodial and custodial models"; Robinhood Earn runs self-custodial embedded wallets in production |
| C-PR-2 | **No Zed signer** — can the intended UX run without any server authorization key or signer capable of unilateral transactions? | Avoids custody/control characterization. | Yes. | Pilot blocker | Open — note: Moreta case study (9/14) shows the stack supports *programmatic initiation + sweeps to an app-controlled wallet*, so absence-of-signer must be explicitly configured and verified |
...
| C-PR-4 | **Open-loop send/receive** — can customers send to / receive from arbitrary OUSD addresses with user signature? | Open-loop wallet truthfulness (R32). | Yes. | Pilot blocker | Open |
...
| C-PR-6 | **Policy controls** — which optional policies can Zed apply *without* obtaining unilateral asset control? | AML/risk controls that don't undermine self-custody. | Only user-approved or non-custodial controls. | Launch blocker | Open — supportive evidence: deck (9/14) claims "granular authorization policies" defining actions/actors/conditions; need the actual policy model |
