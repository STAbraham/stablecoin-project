...
- C-R14. Counsel: DeFi-yield-to-retail characterization (C-Q12); Coins.ph-partnership legal shape incl. whose order the exchange is (C-Q10/OQ-8); **TMMF/SRC §8 — design-blocking (9/23)**; and validation of Zed's position on pending PHP (C-R4b): customer funds held at Coins.ph under its VASP authority, with Zed as interface — counsel to confirm the fact pattern that sustains it and flag anything that would recharacterize it as Zed-administered stored value.
...
Reference ledgers: Coins.ph technical = OQ-1..13 (integration doc) · partner/legal = C-Q6, C-Q10, C-Q12, C-Q14, C-PR-1..5 (counterparty tracker) · product = C-D9/10/12/13/15/16 · research = C-Q15 (→ C-R15).
