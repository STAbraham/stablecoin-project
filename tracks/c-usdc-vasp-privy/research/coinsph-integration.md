...
- **OQ-12.** Reconciliation supports for the master account: is there an API for the master-account **aggregate balance** and/or periodic statements? Can Coins attribute pending PHP to the `coinsUserId` on *their* books (vs. tagging only)? What does Coins recommend merchants use as the recon anchor for deposits-minus-orders?
