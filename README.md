...
The project explores **three alternatives in parallel** alongside the incumbent (PRD v5.1) architecture: **A** OUSD+CASP, **B** OUSD+VASP, **C** USDC via VASP partner + Privy Earn. `tracks/TRACKS.md` is the comparison board and rulebook; each track has `tracks/<track>/track.md` (thesis, Mermaid architecture, track-scoped IDs like `C-Q7`) and `research/` for its digests of vendor docs. Drop inbox items anywhere in `inbox/` — including docs that span tracks; content is routed at processing time (cross-track vendor facts land in the counterparty tracker, which all tracks share; track-specific material digests into that track's `research/`). The `inbox/track-a|b|c/` folders are optional hints. Track lifecycle: exploring → candidate (gets a branded funds-flow diagram) → selected (merges into the PRD) / parked (with post-mortem).
